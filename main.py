...
class RefineRequest(BaseModel):
    requirements: str = Field(..., description="The original system requirements text")
    current_mermaid: str = Field(..., description="The current Mermaid.js code of the diagram")
    instruction: str = Field(..., description="Natural language instructions explaining minor changes to apply")
...
@app.post("/api/refine")
async def refine_dfd(data: RefineRequest):
    endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
    api_key = os.getenv("AZURE_OPENAI_API_KEY")
    ...
    system_prompt = (
        "You are an expert systems architect and diagram engineer.\n"
        "Your task is to refine or modify an existing Data Flow Diagram (DFD) based on a user's natural language instruction for minor changes.\n"
        "You must respond with a JSON object ONLY. Do not include any conversational preamble or postscript.\n\n"
        "You are given:\n"
        "1. The original requirements.\n"
        "2. The current Mermaid.js code of the diagram.\n"
        "3. The user's instruction explaining the minor changes.\n\n"
        "Your output must update the DFD to apply the instruction, while keeping the rest of the diagram intact.\n"
        "The JSON object must have this exact schema:\n"
        "{\n"
        '  "title": "A short, generic, and professional title for the diagram (do not include specific proprietary system names like CCR, etc.)",\n'
        '  "description": "An extremely short, generic, and concise explanation of the data flow (maximum 1 sentence)",\n'
        '  "mermaid": "Valid, updated Mermaid.js flowchart source code applying the user\'s changes. Use flowchart TD or LR. Follow these formatting and shape rules strictly:\\n'
        '    - ALWAYS wrap ALL node labels in double quotes (\\\") inside their shapes to prevent parsing errors.\\n'
        '    - Split long labels (more than 4 words) into multiple lines using HTML breaks <br/> to keep shapes compact and highly readable.\\n'
        '    - Databases, SQL tables, and data stores (cylindrical shape): nodeId[(\\\"Label Text\\\")]\\n'
        '    - Decisions (diamond shape): nodeId{\\\"Label Text\\\"}\\n'
        '    - Processes, jobs, inputs, start, and end nodes (stadium shape): nodeId([\\\"Label Text\\\"])\\n'
        '    - ALWAYS define and apply visual class styling at the end of the Mermaid code to color-code components. Include the following class definitions and apply them to all nodes (do not add any custom fill or stroke colors here, keep them exactly as shown below):\\n'
        '      classDef process stroke-width:2px;\\n'
        '      classDef datastore stroke-width:2px;\\n'
        '      classDef decision stroke-width:2px;\\n'
    ...
    user_prompt = (
        ...
        f"Current Mermaid Code:\n{data.current_mermaid}\n\n"
        f"User Instruction for Minor Changes:\n{data.instruction}\n\n"
        f"Please apply the instruction and return the updated JSON."
    )
